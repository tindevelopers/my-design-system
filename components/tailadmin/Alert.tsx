    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          {title && (
            <h4 className={`font-semibold mb-1 ${variantClasses[variant].title}`}>
              {title}
            </h4>
          )}

          <p className={variantClasses[variant].message}>{message}</p>
        ...
        {closable && (
          <button
            onClick={onClose}
            className={`flex-shrink-0 ${variantClasses[variant].icon} hover:opacity-70 transition-opacity`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
